...
const repo: OtpRepo = {
  ...
  getByEmail: async email => {
    ...
    if (error) throw new Error(error.message)
    const row =
      Array.isArray(data) && data.length > 0 ? (data[0] as OtpRecord) : null
    return row
  },
...
export const generateOtp = async (
  ...
  return { ok: true, status: 200, data: { email: e, otp } }
}
...
export const verifyOtp = async (
  ...
  const e = email.trim().toLowerCase()
  const o = otp.trim()
  if (!isValidEmail(e) || !isValidOtp(o))
  ...
  if (new Date(rec.expiresAt).getTime() < now.getTime())
    return { ok: false, status: 403, error: 'Expired OTP' }
  ...
  .post(
    ...
    async ({ body, set }) => {
      ...
      if (res.ok && res.data?.email) {
        await sendEmail(
          'brand@nigeriacreators.com.ng',
          'New Brand Email Verification Request',
          'brand-verification-notification',
          { email: res.data.email, otp: res.data.otp },
          'brand'
        )
      }
