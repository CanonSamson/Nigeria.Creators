...
export default function BrandDeleteAccountPage () {
  ...
  const handleDelete = async () => {
    try {
      ...
      await supabaseService.client.from('brand_profile').delete().eq('userId', userId)
  ...
  return (
    <div className='max-w-[640px] font-sans space-y-6'>
      <div>
        <h2 className='text-[18px] md:text-[22px] font-bold text-black'>Delete Account</h2>
        <p className='text-text-color-200 text-[14px] md:text-[16px]'>
          This will remove your profile data from Nigeria Creators. Type &rdquo;Delete my account&rdquo; to confirm.
        </p>
      </div>
      ...
      <div className='pt-2'>
        <Button
          ...
          className={`w-auto px-5 py-[12px] text-[16px] font-medium rounded-[12px] ${canDelete ? 'bg-red-500 hover:bg-red-600' : 'bg-red-300'} text-white`}
          buttonType='button'
