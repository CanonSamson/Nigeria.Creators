...
export default function BrandNotificationSettingsPage () {
  ...
  const persistSettings = async ({
    ...
    try {
      ...
      if (existing?.id) {
        ...
        const { error } = await supabaseService.client.from('user_settings').insert(payload)
        if (error) throw new Error(error.message)
      }
    } catch (e) {
  ...
  return (
    <div className=' font-sans max-w-[640px] space-y-6'>
      <div>
        <h2 className='text-[18px] md:text-[22px] font-bold text-black'>Notification Preferences</h2>
        <p className='text-text-color-200 text-[14px] md:text-[16px]'>Control how you receive updates</p>
      </div>
